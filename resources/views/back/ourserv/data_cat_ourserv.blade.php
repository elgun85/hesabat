...
@section('title','data_cat_ourserv')
            ...
            <div class="card">
                <div class="card-header" >
                    <div class="card-title">
                        <form action="{{route('data_cat_ourserv')}}" method="get" name="formdan">
                            @csrf
...
{{--                                    <tr class="text-left
                                    ...
                                    <tr class="text-center">
                                       ...
                                       <td class="text-left">{{$gelir->xidmetin_novu}}
                                        ...
                                        <td>{{number_format(round(($menzil_esas= $gelir->menzil_summa/1.18),2), 2, ',', ' ')}}</td>  {{--esas--}}
                                        <td>{{number_format(round(($menzil_edv= $gelir->menzil_summa-($gelir->menzil_summa/1.18)),2), 2, ',', ' ')}}</td>  {{--edv--}}
                                        <td>{{number_format($gelir->menzil_summa, 2, ',', ' ')}}</td>

                                        <td>{{$gelir->idere_say}}</td>
                                        <td>{{number_format($esas_idare_mebleg=round((($gelir->idere_summa-$gelir->idere_edv)/1.18+$gelir->idere_edv),2), 2, ',', ' ')}}</td>
                                        <td>{{number_format($qur_edv=round(($gelir->idere_summa-round((($gelir->idere_summa-$gelir->idere_edv)/1.18+$gelir->idere_edv),2)),2), 2, ',', ' ')}}</td>
                                        <td>{{number_format($qur_meb=round(($gelir->idere_summa),2), 2, ',', ' ')}}</td>

                                        <td>{{$gelir->cemi_say}}</td>
                                        <td>{{number_format($esas_umumi=round((($gelir->cemi_hesab-$gelir->idere_edv)/1.18+$gelir->idere_edv),2), 2, ',', ' ')}}</td>    {{--esas--}}
                                        <td>{{number_format($edv_umumi=round(($gelir->cemi_hesab-round((($gelir->cemi_hesab-$gelir->idere_edv)/1.18+$gelir->idere_edv),2)),2), 2, ',', ' ')}}</td>  {{--edv--}}
...
{{--senedlesme--}}
                                ...
                                @foreach($senedlesme as $sened)
                                    <tr class="text-center">
                                        ...
                                        <td>{{number_format(round(($menzil_esas= $sened->menzil_summa/1.18),2), 2, ',', ' ')}}</td>  {{--esas--}}
                                        <td>{{number_format(round(($menzil_edv= $sened->menzil_summa-($sened->menzil_summa/1.18)),2), 2, ',', ' ')}}</td>  {{--edv--}}
                                        <td>{{number_format($sened->menzil_summa, 2, ',', ' ')}}</td>

                                        <td>{{$sened->idere_say}}</td>
                                        <td>{{number_format($esas_idare_mebleg=round((($sened->idere_summa-$sened->idere_edv)/1.18+$sened->idere_edv),2), 2, ',', ' ')}}</td>
                                        <td>{{number_format($qur_edv=round(($sened->idere_summa-round((($sened->idere_summa-$sened->idere_edv)/1.18+$sened->idere_edv),2)),2), 2, ',', ' ')}}</td>
                                        <td>{{number_format($qur_meb=round(($sened->idere_summa),2), 2, ',', ' ')}}</td>
