        ...
        <div class="card-body">
         <div class="row">
            ...
            <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">Vəzifəni dəyişmək</h3>
              </div>
              <!-- /.card-header -->
              <!-- form start -->
...
{{--             @endif--}}
              <form  method="POST" action="{{route('mvezife.update',$data->id)}}">
                  @method('PUT')
                  @csrf
                <div class="card-body">
                  <div class="form-group">
                    <label for="exampleInputEmail1"> </label>
                    <input type="text" class="form-control" name="name" id="exampleInputEmail1" value="{{$data->name}}" {{old('name')}} required>
...
{{--    Sag            teref        --}}
  ...
  @foreach($vezifeler as $vezife)
    <tr>
      ...
      <td>{{$vezife->name}}</td>
      <td>
          <a href="{{route('mvezife.index')}}" class="btn btn-outline-success btn-sm" ><i class="fas fa-plus"></i></a>
  ...
  @endforeach
              ...
              {{$vezifeler->links('pagination::bootstrap-4')}}
            </div>
            <!-- /.card -->
          </div>
        </div>
