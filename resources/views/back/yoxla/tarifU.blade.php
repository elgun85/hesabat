...
@section('title','Tarifin dəyikmək')
@section('content')
        ...
        <div class="card-body">
         <div class="row">
            ...
            <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">Tarifi dəyiş</h3>
              </div>
              ...
              <form  method="POST" action="{{route('tarif.update',$tarif->id)}}">
                ...
                <div class="card-body">
                  <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="kod" id="exampleInputEmail1" value="{{$tarif->kod}}" required>
                      @error('kod') <p class="text-danger">{{$message}}</p> @enderror
                  </div>

                  <div class="form-group">
                    <label for="exampleInputEmail1"> </label>
                    <input type="text" class="form-control" name="name" id="exampleInputEmail1" value="{{$tarif->name}}"  autofocus required>
                      @error('name') <p class="text-danger">{{$message}}</p> @enderror
                  </div>

                  <div class="form-group">
                    <label for="exampleInputEmail1"> </label>
                    <input type="text" class="form-control" name="mebleg" id="exampleInputEmail1"  value="{{$tarif->mebleg}}"  autofocus required>
                      @error('mebleg') <p class="text-danger">{{$message}}</p> @enderror
                  </div>

                  <div class="form-group">
                    <label for="exampleInputEmail1"> </label>
                    <input type="text" class="form-control" name="mebleg_q" id="exampleInputEmail1"  value="{{$tarif->mebleg_q}}"  autofocus required>
...
{{--                    <input type="text" class="form-control"  id="exampleInputEmail1" placeholder="Kategoriya yazin" {{old('category')}} required>--}}
                    <select class="form-control" name="category" aria-label="Default select example">
                     <option selected>{{$tarif->category}}  </option>
...
{{--                    <input type="text" class="form-control"  id="exampleInputEmail1" placeholder="Kategoriya yazin" {{old('category')}} required>--}}
                    <select class="form-control" name="novu" aria-label="Default select example">
                     <option selected>{{$tarif->novu}} </option>
                        <option value="Hamısı">Hamısı</option>
                     <option value="Əhali">Əhali</option>
                  ...
                  <div class="form-group">
                    <label for="exampleInputEmail1"> </label>
                    <input type="text" class="form-control" name="qeyd1" id="exampleInputEmail1" value="{{$tarif->name}}"  >
...
{{--    Sag            teref        --}}
  ...
  @foreach($data as $tarif)
    ...
    <td ><input name='id[]' type="checkbox" id="checkItem"  value="{{$tarif->id}}">
      ...
      <td>{{$tarif->qeyd1}}</td>
      <td>
          <a href="{{route('tarif.index')}}" class="btn btn-outline-success btn-sm" ><i class="fas fa-plus"></i></a>
          <a href="{{route('tarif.edit',$tarif->id)}}" class="btn btn-outline-primary btn-sm" ><i class="fas fa-edit"></i></a>
          <a href="{{route('tarif.del',$tarif->id)}}" class="btn btn-outline-danger btn-sm"><i class="fas fa-times "></i> </a>
      </td>
    </tr>
  @endforeach
