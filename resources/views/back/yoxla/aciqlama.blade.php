        ...
        <section class="content">
            ...
            <div class="card">
                <div class="card-header">
                    <div class="card-title"> <form action="{{route('aciqlama')}}" method="get" name="formdan">
                            @csrf
...
{{--                                    <th>Say</th>
                                ...
                                <tbody>
                                @foreach($gelirler as $basliq => $xidmet)
                                    {{--
...
{{--                                        <td>{{$gelir->cemi_say}}</td>
                                        <td>{{$edv_umumi=round(($gelir->cemi_hesab-round((($gelir->cemi_hesab-$gelir->idere_edv)/1.18+$gelir->idere_edv),2)),2)}}</td>
                                        <td>{{$esas_umumi=round((($gelir->cemi_hesab-$gelir->idere_edv)/1.18+$gelir->idere_edv),2)}}</td>--}}
                                       {{-- <td>{{$gelir->cemi_hesab}}</td>--}}

                                        <td>

                                            <ul>
                                                {{$basliq}} - ({{ $xidmet->count() }}) tarif
                                                <ul>

                                                    @foreach($xidmet as $item)
                                                    <li>{{$item->xidmetin_novu}}
                                                    <ul>
                                                        <li>{{$item->kodtarif}} {{$item->adtarif}} <br> <b>{{ $item->cemi_hesab }}</b></li>
                                                    </ul></li>
                                                        @php $say=0;
                                                        $say+=$item->cemi_hesab;
                                                        @endphp
